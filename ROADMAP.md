...
| [066](docs/PRD/066-prd-ai-assisted-suggestions.md) | AI-assisted suggestions | Planned — Phase 0 retrieval tranche and quarantine pipeline merged | Phase 0 retrieval-only `forge author reuse` merged in PR #153 (merge `7d0d9f3`; [reuse plan](docs/plans/2026-09-11-066-mvp-reuse-plan.md), [gate register](docs/authoring-gates.md#gate-reuse--prd-066-phase-0-reuse-tranche)); the five-step `forge suggest` pipeline with recorded-response import (process execution withheld pending OS confinement) merged in PR #155 (merge `f09a299`; [suggestions plan](docs/plans/2026-09-12-066-suggestions-pipeline.md), [GATE-SUGGEST rows](docs/authoring-gates.md#gate-suggest--prd-066-local-suggestion-pipeline)); adjudicated corpora, M-14/M-15 thresholds, usefulness and release gates remain open |
...
| **Later** | PRD 066 | The offline suggestion pipeline is merged (PR #155, merge `f09a299`; recorded-response import only, process execution withheld); adjudicated corpora, quality thresholds and measured usefulness remain the gates before any release, and the provider/privacy question is resolved to a local-only boundary |
