...
**Implementation:** Add the `uuid` crate with the `v5` feature to `Cargo.toml`. Define a `FORGE_NAMESPACE_UUID` constant (generate a v4 UUID once and hardcode it). Implement `normalize_for_hashing` to trim leading/trailing whitespace, collapse any sequence of Unicode whitespace characters (including tabs, newlines, non-breaking spaces) into a single ASCII space, and preserve all other characters — producing deterministic output suitable for hashing. Implement `generate_stable_id` as a deterministic function: normalize text then generate a UUID v5 using `FORGE_NAMESPACE_UUID` as the namespace. Implement `assign_stable_ids` to walk all sections and requirements in a `PolicyDocument` and populate each requirement's `stable_id`.
