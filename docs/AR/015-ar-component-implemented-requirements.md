...
> Use a direct mapping approach: a `build_control_implementations` function that iterates over `PolicyRequirement`s, maps each to an `implemented-requirement` entry with a deterministic UUID v5, a `control-id` derived from the requirement's stable ID, and a `description` containing the requirement prose as the implementation narrative. All implemented-requirements are grouped under a single `control-implementations` entry whose `source` references the `--source-profile` value.

### TL;DR for Agents 🟡 `@human-review`
> Implemented-requirements are generated by a `build_control_implementations` function that takes `&[PolicyRequirement]`, the `--source-profile` value, and the policy title. It produces a `serde_json::Value` array containing one `control-implementations` entry with: a deterministic UUID v5, the `source` profile reference, a `description` indicating the policy context, and an `implemented-requirements` array where each entry has a UUID v5, a `control-id` (from the requirement's stable_id), and a `description` (the requirement prose). The output is injected into the Component Definition's documentary component from WI-14. Do NOT validate control-ids against the source profile (deferred to WI-19). Do NOT use `remarks` for implementation narratives — use `description`. Do NOT generate random UUIDs — all must be deterministic v5.
...
graph TD
    subgraph "oscal module — implemented requirements"
        ...
        CIDGEN[control_id_from_stable_id\nshared with WI-9]
    end
...
|-----------|---------------|-----------|--------------|
| `build_control_implementations` | Produce `control-implementations` JSON array | `fn(&[PolicyRequirement], &str, &str) -> Result<Value, ForgeError>` | `map_requirement_to_implemented`, `uuid` |
| `map_requirement_to_implemented` | Map a single PolicyRequirement to implemented-requirement JSON | `fn(&PolicyRequirement) -> Result<Value, ForgeError>` | `control_id_from_stable_id`, `uuid` |
| `control_id_from_stable_id` | Generate consistent control-id from stable_id | Shared utility (also used by WI-9) | None |
...
sequenceDiagram
    ...
    participant CID as control_id_from_stable_id
    participant UUID as uuid::Uuid

    BCD->>BCI: build_control_implementations(requirements, source_profile, title)
    ...
    loop for each PolicyRequirement
        ...
        MRI->>CID: control_id_from_stable_id(requirement.stable_id)
        CID-->>MRI: "POL-AC-001" (or similar)
...
pub fn map_requirement_to_implemented(
    ...
    let control_id = control_id_from_stable_id(&requirement.stable_id);
    let ir_uuid = generate_impl_req_uuid(requirement);
...
/// Generate a consistent control-id from a requirement's stable_id.
/// This function is shared with the Catalog builder (WI-9/WI-10).
fn control_id_from_stable_id(stable_id: &str) -> String {
    // Reuse the same ID scheme as Catalog controls
    stable_id.to_string()
}
...
```
1. control_id_from_stable_id is used by both:
   a. Catalog builder (WI-9) — to set control.id
   b. This WI — to set implemented-requirement.control-id
2. Shared function ensures cross-artifact consistency:
...
- Shared `control_id_from_stable_id` utility function (must be accessible to both WI-9 and WI-15 code)
...
- **Shares:** `control_id_from_stable_id` with WI-9 (Catalog builder)
...
|------|------------|--------|------------|
| Control-id scheme diverges between WI-9 and WI-15 | Med | Med | Shared `control_id_from_stable_id` utility function ensures consistency |
...
2. Extract `control_id_from_stable_id` into a shared utility module (used by WI-9 and WI-15)
...
- **Don't:** Create a separate control-id generation function that differs from WI-9's
  ...
  - **Instead:** Share a single `control_id_from_stable_id` utility
...
| M-7 | Consistency | Option 1: ✅ | `control_id_from_stable_id` | Shared utility with WI-9 |
