...
///
/// # Errors
/// Returns [`ForgeError::YamlSerialization`] if `serde_yaml::to_string()` fails.
pub fn serialize_to_yaml<T: Serialize>(model: &T) -> Result<String, ForgeError> {
    ...
    let result = serde_yaml::to_string(model).map_err(|source| ForgeError::YamlSerialization {
...
///
/// # Errors
/// Returns [`ForgeError::YamlSerialization`] if `serde_yaml::from_str()` fails.
pub fn deserialize_from_yaml<T: DeserializeOwned>(yaml: &str) -> Result<T, ForgeError> {
    serde_yaml::from_str(yaml).map_err(|source| ForgeError::YamlSerialization {
...
mod tests {
    ...
    #[test]
    fn deserialization_preserves_yaml_error_source() {
        let error = deserialize_from_yaml::<serde_json::Value>("invalid: [yaml").unwrap_err();
        let ForgeError::YamlSerialization { context, source } = error else {
            panic!("expected YAML serialization error");
        };
