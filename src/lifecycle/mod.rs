...
pub fn execute_init(options: &InitOptions<'_>) -> Result<(), ForgeError> {
    ...
    let record = LifecycleRecord {
        ...
        policy: PolicyIdentity {
            ...
            owner_keys: options.owners.to_vec(),
            source,
            generated_artifacts,
        },
        parties,
...
pub fn execute_attest(record_path: &Path, output: Option<&Path>) -> Result<(), ForgeError> {
    ...
    let attestation = ApprovalAttestation {
        schema_version: "forge.policy-approval-attestation/1",
        policy: PolicyReference {
...
pub fn execute_transition(options: &TransitionOptions<'_>) -> Result<(), ForgeError> {
    ...
    let assertions = parse_assertions(options.assertions)?;
...
fn parse_parties(specs: &[String]) -> Result<Vec<Party>, ForgeError> {
    ...
    for spec in specs {
        ...
        let roles = roles.split(',').map(parse_role).collect::<Result<Vec<_>, _>>()?;
...
fn current_fingerprints(
    ...
    generated.sort();
    Ok(FingerprintSet { source_sha256: source.sha256, generated_artifacts: generated })
}
...
fn status_report(
    ...
    if record.state == LifecycleState::Approved
        && approved.as_ref().is_some_and(|value| value != &current)
    {
...
fn gate_action_required(reports: &[StatusReport], gate: &LifecycleGate) -> bool {
    match gate {
        ...
        LifecycleGate::Publication => reports
            .iter()
            .any(|report| report.state != LifecycleState::Approved || !report.blockers.is_empty()),
    }
}
...
fn build_queue(reports: Vec<StatusReport>, as_of: NaiveDate) -> QueueReport {
    ...
    let groups = grouped
        ...
        .collect();
    QueueReport {
        schema_version: "forge.policy-lifecycle-queue/1",
        as_of,
        groups,
        trust_boundary: TRUST_BOUNDARY,
    }
