...
(async()=>{
 ...
 try {
   ...
   if(readOnly){assert.equal(await page.getByRole("button",{name:"Preview registration"}).count(),0);}
     ...
     await register("framework.json","oscal-catalog-artifact","framework");
     ...
     await page.getByLabel("Reviewed policy subject").selectOption({index:1});await page.getByLabel("Reviewed framework subject").selectOption({index:1});await page.getByLabel("Reviewed relationship",{exact:true}).selectOption("no-relationship");
