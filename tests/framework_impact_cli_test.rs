...
fn write_disposition_fixture(path: &Path, prior_bytes: &[u8], prior_report: &Value) {
    ...
    write_json(
        ...
        &json!({
            ...
            "prior_report_sha256": hex::encode(Sha256::digest(prior_bytes)),
            "dispositions": [
                {
...
fn traverses_exact_mapping_dependencies_with_stable_paths_and_priorities() {
    ...
    let old_hash = hex::encode(Sha256::digest(std::fs::read(dir.path().join("old.json")).unwrap()));
    let new_hash = hex::encode(Sha256::digest(std::fs::read(dir.path().join("new.json")).unwrap()));
    write_json(
        &manifest_path,
        &impact_manifest(
            &old_hash,
