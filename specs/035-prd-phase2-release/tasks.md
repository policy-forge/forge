...
- [X] T044 [P] [US2] Implement `profile_include_nonexistent_id_produces_error` in `tests/integration_profile_e2e.rs`: generate catalog; run `forge profile --catalog <path> --include NONEXISTENT-CONTROL-999 --format json`; assert exit 0 and NONEXISTENT-CONTROL-999 appears in `imports[0].include-controls[0].with-ids` (permissive behavior per EC-2 clarification; OSCAL profile imports are URI references, not validated Catalog handles) (traces EC-2)
- [X] T046 [P] [US2] Implement `profile_set_param_nonexistent_id_exits_zero` in `tests/integration_profile_e2e.rs`: generate catalog; run `forge profile --catalog <path> --include <id> --set-param nonexistent-param-999 42 --format json`; assert exit 0 and `profile.modify.set-parameters` contains an entry with `param-id: "nonexistent-param-999"` and value `"42"` (permissive: param IDs are OSCAL Profile-level, independent of Catalog param IDs) (traces EC-3)
- [X] T047 [P] [US2] Implement `profile_include_and_exclude_rejected` in `tests/integration_profile_e2e.rs`: attempt to run `forge profile --catalog <path> --include <id> --exclude <id2> --format json`; assert exit non-zero (clap rejects mutually-exclusive flags) and stderr contains a usage error message — `--include` and `--exclude` are declared `conflicts_with` each other in the CLI (traces EC-6)
...
**Checkpoint**: US2 complete — `cargo test integration_profile_e2e` → 8 passed, 0 failed (5 profile E2E tests + T044 EC-2 permissive ID test + T046 EC-3 nonexistent param test + T047 EC-6 mutually-exclusive flags test).
